...
public class ShipmentAdapter extends RecyclerView.Adapter<ShipmentAdapter.ViewHolder> {
    ...
    List<BookingDetails.Item> shipmentItems=new LinkedList<>();
    Context context;
    OnItemClickListener onItemClickListener;

    public ShipmentAdapter(Context context, List<BookingDetails.Item> shipmentItems, OnItemClickListener onItemclickListener){
    ...
    public void onBindViewHolder(ShipmentAdapter.ViewHolder holder, final int position) {
        ...
        holder.load_shipment.setVisibility(View.GONE);
        if(position==shipmentItems.size()-1){
            holder.divider.setVisibility(View.GONE);
        }

        List<BookingDetails.Issue> issues=shipmentItems.get(position).getIssues();
        if(issues!=null){
            holder.damage_status.setVisibility(View.VISIBLE);
        }else{
            holder.damage_status.setVisibility(View.INVISIBLE);
        }
        Glide.with(context).load(Uri.parse("android.resource://com.shwetak3e.deliveryapp/" + R.drawable.ic_refrigerator).toString()).into(holder.commodity_img);
        holder.shipment_id.setText(shipmentItems.get(position).getId());
        holder.total_count.setText(shipmentItems.get(position).getItemCount().toString());
        holder.loaded_count.setText(shipmentItems.get(position).getLoadedCount().toString());
        holder.missing_count.setText("0");
